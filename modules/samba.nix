    ...
    services.samba = {
      ...
      settings = {
        global = {
          ...
          "hosts allow" = "${cfg.allowedHosts} 127.0.0.1 localhost";
        ...
        "private" = {
          ...
          "create mask" = "0644";
          "directory mask" = "0755";
          "force user" = config.local.users.default.user;
          "force group" = config.local.users.media.group;
          "valid users" = config.local.users.default.user;
        };
      };
    };
    ...
    system.activationScripts = {
      ...
      init_smbpasswd.text = ''
        /run/current-system/sw/bin/printf "$(/run/current-system/sw/bin/cat ${
          ...
          config.sops.secrets."samba/password".path
        })\n" | /run/current-system/sw/bin/smbpasswd -sa ${config.local.users.default.user}
      '';
    };
